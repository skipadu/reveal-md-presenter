...
export default class RevealMdPresenterUI extends React.Component {
  ...
  async showPresenterWindow() {
    ...
    // TODO: Testing the child_process stuff
    // TODO: using which to see where reveal-md is installed if any
    // FIXME: does not seem to work like this when nvm is used as the
    ...
    commandExists('reveal-md', function (err, commandExists) {
      ...
      if (commandExists) {
        console.log('reveal-md was found!');
      }
      console.log('commandExist ran but... no results?');
    });
    const revealMd = exec('reveal-md "/Users/sk/Library/Application Support/inkdrop/tmpPresentation.md" -w --port 1948 --disable-auto-open', function (
      error,
      stdout,
      stderr
    ) {
      if (error) {
        console.log('Error when tried to start reveal-md', error);
      }
      console.log('which STDOUT:', stdout);
      console.log('which STDERR:', stderr);
    });
    revealMd.on('exit', function (code) {
