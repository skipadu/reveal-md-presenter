...
export default class RevealMdPresenterUI extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      manualRevealMdStart: inkdrop.config.get(
        'reveal-md-presenter.manualRevealMdStart'
      ),
    };
    ...
    inkdrop.config.onDidChange(
      'reveal-md-presenter.manualRevealMdStart',
      ({ newValue, oldValue }) => {
        this.setState({ manualRevealMdStart: newValue });
      }
    );
  ...
  componentWillMount() {
    ...
    this.subscriptions.add(
      inkdrop.commands.add(document.body, {
        ...
        'core:save-note': () => this.documentWasSaved(),
      })
    );
  }
  ...
  async showPresenterWindow() {
    ...
    if (!isRevealMdCommandFound) {
      ...
      if (!revealMdRunning) {
        if (!this.state.manualRevealMdStart) {
          this.startRevealMd();
        }
      ...
      <HelpDialog ref="helpDialog" title="How to use Presenter Mode">
        <div className="reveal-md-presenter help-dialog">
          <div>
            ...
            <div>
              <h3>Manual starting of reveal-md</h3>
              If you are using Windows or you just want to start reveal-md by
              yourself you first need to set it enabled from the plugin config.
              <p>
                To start the <b>reveal-md</b>, copy the command below and paste
                it to your terminal/command line and run it.
              </p>
            </div>
          ...
          <div>
            ...
            <p>
              ...
              saved to temporal file. On macOS reveal-md is started
              automatically. If the Presenter Mode is already open, it just
              saves and reveal-md's watch options should make sure that the
