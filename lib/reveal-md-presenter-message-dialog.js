...
export default class RevealMdPresenterMessageDialog extends React.Component {
  ...
  async createWindow(url) {
    ...
    // TODO: ready to show -event to not get "flickering"? if even any happening...
    //
  ...
  async showPresenterWindow() {
    ...
    if (!this.state.isServerRunning) {
      ...
      try {
        const response = await fetch_retry(urlToOpen, 5);
      ...
      if (!revealMdServerResponding) {
        ...
        const errorNotification = inkdrop.notifications.addError(
          'Could not connect to reveal-md',
          {
            detail: 'Click here to show the Help-dialog',
            dismissable: true,
          }
        );
  ...
  getCommandToCopy() {
    ...
    return `reveal-md "${this.getTempFilepath()}" -w --port ${selectedPort} --disable-auto-open --separator "^\n===\n" --vertical-separator "^\n\|\|\|\n"`;
  }
  ...
  render() {
    ...
    return (
      <MessageDialog ref="helpDialog" title="How to use Presenter Mode">
