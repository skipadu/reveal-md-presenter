...
// /**
//  * @typedef {Object} ProcessLookupResult
//  * @property {boolean} found - Was the process found
//  * @property {string} error - Error if any
//  */
// /**
//  * @param {string} command
//  * @param {string[]} argsArr
//  * @return {Promise<ProcessLookupResult>} - Promise
//  */
// function promiseProcessLookup(command, argsArr) {
...
const fetch_retry = async (url, retries) => {
  try {
    console.log(`fetch_retry. Retries (decreasing) #${retries}`);
...
export default class RevealMdPresenterMessageDialog extends React.Component {
  ...
  async toggle() {
    ...
    if (!serverRunning) {
      ...
      //   '--disable-auto-open',
      // ]);
      // FIXME: use delay if not able to do it otherwise
      // await this.sleepForAwhile(2000);
      // const isAlive = true;

      //
      //
      //
      // Works, but not enough to check if server is alive
      // const command = 'Inkdrop';
      // const args = [
      ...
      // const { found, error } = await promiseProcessLookup(command, args);
      // console.log('is found?: ', found);
      // const isAlive = found;
      // if (!found) {
      //   console.log('error from lookup:', error);
      // }
      // const endDate = new Date();
      // const totalTimeSpentInMs = endDate.getTime() - startDate.getTime();
      // console.log(
      //   `Time used before available / exited: ${totalTimeSpentInMs}ms`
      // );
      //
      //
      //
      ...
      // const isAlive = await ping.promise.probe(revealMdHost, { timeout: 10 });
      ...
      if (isAlive) {
        ...
        // TODO: test with fetch, is the server already up
        //
        //
        //
        // TODO: make a method to try few times if not responding, 50ms between, 20x times? :D
